    ...
    <div className="flex flex-1 mx-auto max-w-container-max w-full px-margin-mobile md:px-margin-desktop gap-gutter pt-stack-lg pb-10 md:pb-stack-lg">
      ...
      <main className="flex-1 flex flex-col min-w-0 pr-0 md:pr-4">
        ...
        {visibleJobs.length > 0 ? (
          <>
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 ${hasMore ? 'mb-6 sm:mb-8' : ''}`}>
              {visibleJobs.map(job => (
            ...
            {hasMore && (
              <div className="flex justify-center">
                <button
                  ...
                  className="border border-outline-variant/80 hover:border-primary text-on-surface hover:text-primary font-semibold px-4 py-1.5 sm:px-6 sm:py-2 rounded-full text-[11px] sm:text-xs transition-all cursor-pointer shadow-2xs hover:shadow-xs active:scale-95"
                >
                  Muat Lebih Banyak
                </button>
              </div>
